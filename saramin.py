...
import pandas as pd # df정리
import re #정규표현식(내용을 예쁘게 다듬을 때 사용)
from html import unescape #html형식에서 깨진 글자를 복원
...
def crawling_data(search_word):
    ...
    #2. requests.get(주소) => 그 '주소'에 있는 html을 받아옴
    # html : 인터넷에서 정보를 표현하는 파일 형식
    #3. bs4 -> 받아온 html을 파싱(parsing) , 필요정부 추출
    #4. bs4.select(구분자) <div>, </div> 등
    #5. bs4.select_one(구분자)
    #6. 얻어낸 정보 -> re(정규표현식)으로 '정제' or pandas로 정리해서 저장


    #검색창과 결과 페이지가 같이 나오는걸 찾아서 뒤부터 소거해가면됨.
    base_url = 'https://www.saramin.co.kr/zf_user/search'
    #헤더 : 브라우저가 서버에 무언가 요청할때 '나 이런 브라우저에요'라는 내용
    headers = {
        #요청하는 브라우저의 종류가 Mozilla/AppleWebKit/Chrome/Safari 중에 있다는 것
        "User-Agent": (
    ...
    #params : 웹페이지에 이런 정보를 요청합니다 하는 '검색어, 검색조건'
    #timeout : 웹페이지로부터 회신이 올 때까지 기다리는 시간 최대치
    response = requests.get(base_url, headers = headers, 
                            params = {'searchword' : search_word},
                            timeout = 10)

    #만약 response가 <200> 이라면 => 정상 /400이나 500으로 시작하면 에러
    #400에러 : 서버에러 #500에러 : 클라이언트 에러
    ...
    #soup.select(구분자) : '구분자'가 들어가는 모든 내용을 select
    #find_all과의 차이점 : find가 옛날 방식. 전통적인 선택방식 / select가 현대적
    #soup.find_all('div', class_ = 'item_recruit')와 동일
    for item in soup.select('div.item_recruit'):
        #1. 회사명
        #select_one(구분자) : 구분자 이름을 갖는 딱 하나만 가져오란 뜻
        ...
        #rows 리스트, 리스트 안에 딕셔너리 -> 판다스로 만들기위해
        rows.append({
            '공고 이름' : job_title,
            '회사 위치' :  location, 
            '조건 1' : condition1, 
    ...
    #최종적으로 얻어진 rows를 pd.DataFrame으로 감싸서 df로 맏늘어줌
